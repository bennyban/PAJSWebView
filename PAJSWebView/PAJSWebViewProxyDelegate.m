...
/*
 This is the content of pajs-inject.js
 Putting it inline in order to prevent loading from files
 */
NSString* INJECT_JS = @"window.PAJS = {\
__callbacks: {},\
\
...
@interface PAJSWebViewProxyDelegate ()

@property (nonatomic, retain) NSMutableDictionary* javascriptInterfaces;  /**< 数据字典 */

@end

@implementation PAJSWebViewProxyDelegate

@synthesize javascriptInterfaces, realDelegate;
...
- (BOOL)webView:(UIWebView *)webView shouldStartLoadWithRequest:(NSURLRequest *)request navigationType:(UIWebViewNavigationType)navigationType{
    ...
    if ([requestString hasPrefix:@"pa-js:"]) {
        /*
         A sample URL structure:
         pa-js:MyJSTest:test
         pa-js:MyJSTest:testWithParam%3A:haha
         */
        ...
        if ([components count] > 3){
            ...
            [invoker setArgument:&webView atIndex:2];
            for (NSUInteger i = 0, j = 0, l = formattedArgs.count; i < l; i+=2, j++)
            {
                ...
                {
                    PAJSDataFunction* func = [[PAJSDataFunction alloc] initWithWebView:(PAJSWebView *)webView];
                    func.funcID = argStr;
                    [args addObject:func];
                ...
                else if ([@"s" isEqualToString:type])
                {
                    ...
                    [args addObject:arg];
                    [invoker setArgument:&arg atIndex:(j + 3)];
                }
            }
        }
        ...
        if ([sig methodReturnLength] > 0){
            ...
            if (retValue == NULL || retValue == nil){
                [webView stringByEvaluatingJavaScriptFromString:@"PAJS.retValue=null;"];
            }else{
                ...
                [webView stringByEvaluatingJavaScriptFromString:[@"" stringByAppendingFormat:@"PAJS.retValue=\"%@\";", retValue]];
            }
        }
...
- (void)webViewDidStartLoad:(UIWebView *)webView{
    ...
    for(id key in self.javascriptInterfaces) {
        ...
        [injection appendString:@"PAJS.inject(\""];
